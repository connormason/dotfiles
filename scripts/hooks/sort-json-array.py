#!/usr/bin/env python3
"""
Pre-commit hook to sort a top-level JSON array of objects by a chosen key.
"""
# /// script
# requires-python = ">= 3.9"
# dependencies = []
...
def parse_args(argv: Sequence[str] | None = None) -> Args:
    ...
    parser = argparse.ArgumentParser(
        description='Sort a top-level JSON array of objects by a chosen key',
    ...
    def add_positional_args() -> None:
        ...
        positional_args.add_argument(
            'filenames',
            nargs='*',
            help='JSON files (top-level array of objects) to sort',
        )
    add_positional_args()

    def add_sort_opts() -> None:
        """
        Register the sorting options group on the parser.
        """
        sort_opts = parser.add_argument_group('Sorting options')
        sort_opts.add_argument(
            '-k',
            '--key',
            default=DEFAULT_KEY,
            help=f'Object key whose value each array entry is sorted by (default: {DEFAULT_KEY!r})',
        )
        sort_opts.add_argument(
            '--indent',
            type=int,
            default=DEFAULT_INDENT,
            help=f'Number of spaces to indent rewritten JSON by (default: {DEFAULT_INDENT})',
        )
    add_sort_opts()
    ...
    def add_output_opts() -> None:
        ...
        output_opts.add_argument(
            ...
            '--verbose',
            action='store_true',
            help='Output sort result for each analyzed file to console',
        )
    add_output_opts()
...
def sort_file(path: Path, *, key: str, indent: int) -> int:
    """
    Sort a single JSON file's top-level array of objects by ``key`` in place.

    The file is rewritten only when the sorted, re-serialized content differs from the original. Output matches
    the ``pretty-format-json`` hook (sorted object keys, trailing newline) so the two never produce competing rewrites.

    :param path: path to the JSON file to sort
    :param key: object key whose value each array entry is sorted by
    :param indent: number of spaces to indent the rewritten JSON by
    :return: 0 if already sorted, 1 if the file was rewritten or could not be processed
    """
    try:
        original = path.read_text(encoding='utf-8')
    except Exception as open_exc:
        printf(f'{ERROR}   {style(path, fg="bright_white")}')
        printf(f'Unable to load file: {open_exc!s}', fg='bright_red', indent=8)
        return 1
    ...
    try:
        ...
        printf(f'{ERROR}   {style(path, fg="bright_white")}')
        printf(f'Unable to decode JSON: {json_exc!s}', fg='bright_red', indent=8)
        return 1

    if (not isinstance(data, list)) or not all(isinstance(entry, dict) for entry in data):
        printf(f'{ERROR}   {style(path, fg="bright_white")}')
        printf('Expected a top-level array of objects', fg='bright_red', indent=8)
        return 1

    ordered     = sorted(data, key=lambda entry: entry.get(key, ''))
    sorted_text = json.dumps(ordered, indent=indent, ensure_ascii=True, sort_keys=True, separators=(',', ': ')) + '\n'

    if sorted_text == original:
        if VERBOSE:
            printf(f'{INFO}    {style(path, fg="bright_white")}')
            printf(style(f'Already sorted by {key!r}', fg='green'), indent=8)
        return 0

    path.write_text(sorted_text, encoding='utf-8')
    printf(f'{INFO}    {style(path, fg="bright_white")}')
    printf(style(f'Sorted array by {key!r}', fg='yellow'), indent=8)
    return 1


def main(argv: Sequence[str] | None = None) -> int:
    """
    Sort top-level JSON arrays of objects by a chosen key.

    :param argv: command-line arguments (default: ``sys.argv[1:]``)
    :return: 0 if every file was already sorted, non-zero if any was rewritten or failed
    """
    args = parse_args(argv)
    if not args.filenames:
        printf('No filenames to sort', fg='yellow')
        return 0

    retval: int = 0
    for filename in args.filenames:
        retval |= sort_file(Path(filename), key=args.key, indent=args.indent)
    return retval
