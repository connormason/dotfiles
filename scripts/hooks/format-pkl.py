#!/usr/bin/env python3
"""
Pre-commit hook to auto-format Pkl configuration files via ``pkl format``.

If ``pkl`` is not found on PATH, prints a warning and exits successfully (exit code 0) so that the hook does not
block commits in environments where ``pkl`` is not installed. Otherwise, runs ``pkl format`` on all provided files
...
MODIFIED   = style('MODIFIED',   fg='bright_yellow')
UNMODIFIED = style('UNMODIFIED', fg='bright_blue')
...
GrammarVersion = Literal[1, 2]

DEFAULT_GRAMMAR_VERSION: GrammarVersion = 2
...
def parse_args(argv: Sequence[str] | None = None) -> Args:
    ...
    parser = argparse.ArgumentParser(
        description='Format Pkl files using ``pkl format``',
    ...
    def add_grammar_options() -> None:
        """
        Register the grammar version options group on the parser.
        """
        grammar_opts = parser.add_argument_group('Grammar options')
        grammar_opts.add_argument(
            '--grammar-version',
            type=int,
            choices=get_args(GrammarVersion),
            default=DEFAULT_GRAMMAR_VERSION,
            help=(
                f'The grammar compatibility version to use\n'
                f'  1: 0.25 - 0.29\n'
                f'  2: 0.30+ {style("(default)", dim=True, fg="bright_white")}'
            ),
        )
    add_grammar_options()
    ...
    def add_output_opts() -> None:
        ...
        output_opts.add_argument(
            ...
            action='store_true',
            default=False,
            help='Output formatting result for each processed file to console',
        )
    add_output_opts()
...
def main(argv: Sequence[str] | None = None) -> int:
    """
    Format Pkl files using ``pkl format``.
    ...
    if not args.filenames:
        print(style('No filenames to format', fg='yellow'))
        return 0
    ...
    if not shutil.which(args.executable):
        if args.require_pkl:
            ...
            printf(f'Install pkl: {PKL_INSTALLATION_DOCS_URL}', indent=12)
            return 1
        printf(
            f'{WARNING}     '
        ...
        printf('Skipping Pkl formatting',                  indent=12)
        printf(style(PKL_INSTALLATION_DOCS_URL, dim=True), indent=12)
        return 0

    base_cmd: list[str] = [args.executable, 'format', '--diff-name-only']
    if args.grammar_version:
        base_cmd.extend(['--grammar-version', str(args.grammar_version)])

    retval: int = 0
    for filename in args.filenames:
        try:
            try:
                subprocess.run([*base_cmd, filename], check=True, capture_output=True, text=True)
            except subprocess.CalledProcessError as cmd_exc:
                if cmd_exc.returncode == 11:
                    subprocess.run([*base_cmd, filename, '--write'], check=True, capture_output=True, text=True)
                    print(f'{MODIFIED}    {style(filename, fg="bright_white")}')
                    continue
                raise

        except subprocess.CalledProcessError as cmd_exc:
            print(f'{ERROR}       {style(filename, fg="bright_white")}')
            if cmd_exc.stderr:
                stderr_lines = [
                    line for line in cmd_exc.stderr.splitlines()
                    if line.strip() and (line.strip() != 'An error occurred during formatting.')
                ]
                printf('\n'.join(stderr_lines), indent=12)
            else:
                printf(str(cmd_exc),            indent=12)

            retval = max(retval, cmd_exc.returncode)

        except Exception as e:
            printf(f'{ERROR}       {style(filename, fg="bright_white")}')
            printf(f'{e!s}', indent=12)
            return 1

        else:
            if args.verbose:
                printf(f'{UNMODIFIED}  {style(filename)}')
