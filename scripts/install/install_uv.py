...
def get_common_install_locations() -> list[Path]:
    ...
    # macOS pip ``--user`` installs land in ``~/Library/Python/<X.Y>/bin/uv`` (one entry per framework Python).
...
def register_pip_install_location(python_exe: str) -> None:
    ...
    Asks the interpreter for its ``sysconfig`` scripts path and appends ``<scripts>/uv`` to the module-level
